...
def parseTopTags(toptags):
	data = dict()
	toptagslist = str(toptags).split('TopItem')
	for num in range(1,len(toptagslist)):
		print(num)
		print(toptagslist[num])
		front = toptagslist[num].replace('(item=pylast.Tag(u\'','')
		print('front: ', front)
		middle = front.split('\'')[0]
		print('middle: ', middle)
		back = front.split('weight=u\'')[1]
